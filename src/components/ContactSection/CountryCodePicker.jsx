...
// Comprehensive list: { name, iso2, dial }
// iso2 = ISO 3166-1 alpha-2 code (used for flagcdn.com)
// dial = country calling code
const ALL_COUNTRIES = [
    ...
    { name: "Antigua & Barbuda", iso2: "ag", dial: "+1268" },
    ...
    { name: "Saint Kitts & Nevis", iso2: "kn", dial: "+1869" },
    { name: "Saint Lucia", iso2: "lc", dial: "+1758" },
    { name: "Saint Vincent & Grenadines", iso2: "vc", dial: "+1784" },
    ...
    { name: "Trinidad & Tobago", iso2: "tt", dial: "+1868" },
...
const getFlagUrl = (iso2) =>
    `https://flagcdn.com/w20/${iso2.toLowerCase()}.png`;
...
const CountryCodePicker = ({ value, onChange }) => {
    ...
    const selected =
        ALL_COUNTRIES.find((c) => c.dial === value) || ALL_COUNTRIES.find((c) => c.iso2 === "in");
    ...
    return (
        <div
            className={`ccp-container${isOpen ? " ccp-open" : ""}`}
            ref={containerRef}
        ...
        >
            ...
            >
                ...
                <span className="ccp-dial">{selected?.dial ?? "+91"}</span>
                <ChevronDown
                    size={13}
                    className={`ccp-chevron${isOpen ? " ccp-chevron-open" : ""}`}
                />
            </button>

            {/* Dropdown */}
            {isOpen && (
                <div className="ccp-dropdown" role="listbox" data-lenis-prevent="true">
                    {/* Search */}
                    <div className="ccp-search-wrap">
                        <Search size={13} className="ccp-search-icon" />
                        <input
                            ref={searchRef}
                            type="text"
                    ...
                    <ul className="ccp-list">
                        {filtered.length === 0 ? (
                            <li className="ccp-empty">No results found</li>
                        ) : (
                            filtered.map((country) => (
                                <li
                                    key={`${country.iso2}-${country.dial}`}
                                    className={`ccp-item${country.dial === value ? " ccp-item-active" : ""}`}
                                    role="option"
                                ...
                                >
                                    <img
                                        ...
                                        loading="lazy"
                                    />
                                    <span className="ccp-item-name">{country.name}</span>
                                    <span className="ccp-item-dial">{country.dial}</span>
                                </li>
                            ))
                        )}
                    </ul>
